...
namespace CoreServices.SearchServices.LCSSearch
{
	public static class SearchService
	{
		...
		/// <returns></returns>
		public static IEnumerable<T> SearchInCollection<T>(IEnumerable<T> collection, string targetProperty,
			string query)
		{
			var searchList = collection.Select(x => new KeyVal<T, int>(x, 0)).ToList();
		...
		/// <param name="query"></param>
		/// <returns></returns>
		public static IEnumerable<T> SearchInCollection<T>(IEnumerable<T> collection,
			Func<T, string> targetPropertyFunc,
			string query)
		{
			var searchList = collection.Select(x => new KeyVal<T, int>(x, 0)).ToList();
			foreach (var pair in searchList)
